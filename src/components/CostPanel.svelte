...
{#if inline}
  <div class="w-full max-w-xl rounded-xl border bg-white shadow-xl">
    <div class="flex items-center justify-between border-b px-5 py-4">
      <div>
        <p class="text-xs uppercase tracking-wide text-gray-500">{$t('Cost breakdown')}</p>
    ...
    <div class="divide-y overflow-y-auto max-h-[70vh]">
      <section class="px-5 py-4 space-y-2">
        <div class="flex items-center justify-between">
          <h4 class="text-sm font-semibold text-gray-800">{$t('Panels')}</h4>
          <span class="text-sm text-gray-600">{$t('Boards × cost')}</span>
        </div>
        <div class="space-y-2">
          {#each summary.materials as m}
            <div class="flex items-center justify-between rounded border border-gray-100 px-3 py-2">
              ...
              <div class="text-sm text-gray-700 flex items-center gap-2">
                <span class="rounded bg-blue-50 px-2 py-0.5 text-blue-800 text-xs font-semibold">{m.boards} {$t('boards')}</span>
      ...
      <section class="px-5 py-4 space-y-2">
        <h4 class="text-sm font-semibold text-gray-800">{$t('Hardware')}</h4>
        <div class="grid grid-cols-2 gap-x-6 gap-y-2 text-sm text-gray-700">
          <div class="flex justify-between"><span>{$t('Screws')}</span><span>{summary.bom.screws}</span></div>
          <div class="flex justify-between"><span>{$t('Wood dowels')}</span><span>{summary.bom.dowels}</span></div>
          <div class="flex justify-between"><span>{$t('Hinges')}</span><span>{summary.bom.hinges}</span></div>
          <div class="flex justify-between"><span>{$t('Drawer slides')}</span><span>{summary.bom.slides}</span></div>
        </div>
      </section>

      <section class="px-5 py-4">
        <div class="flex items-center justify-between">
          <div class="text-sm font-semibold text-gray-800">{$t('Grand total')}</div>
